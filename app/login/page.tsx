...
function LoginForm() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center overflow-hidden px-6 py-10 text-white">
      {/* Flowing animated blue gradient — covers the whole page */}
      <motion.div
        className="absolute inset-0"
        style={{
          backgroundImage:
            "linear-gradient(135deg, #0b1d4a 0%, #13306f 18%, #1E3A8A 34%, #2450a8 50%, #3b5fd9 64%, #1E3A5F 80%, #0b1d4a 100%)",
      ...
      <motion.div
        className="absolute -left-24 top-10 h-96 w-96 rounded-full bg-gradient-to-br from-cyan-400/40 via-blue-400/20 to-transparent blur-3xl mix-blend-screen"
        animate={{ x: [0, 50, -20, 0], y: [0, -30, 25, 0], scale: [1, 1.15, 0.95, 1] }}
        transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute -right-24 top-1/4 h-[28rem] w-[28rem] rounded-full bg-gradient-to-br from-violet-500/40 via-indigo-500/20 to-transparent blur-3xl mix-blend-screen"
        animate={{ x: [0, -35, 30, 0], y: [0, 35, -20, 0], scale: [1, 0.9, 1.1, 1] }}
        transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute -bottom-32 left-1/3 h-[26rem] w-[26rem] rounded-full bg-gradient-to-br from-blue-400/30 via-cyan-300/15 to-transparent blur-3xl mix-blend-screen"
        animate={{ x: [0, 30, -25, 0], y: [0, -20, 20, 0] }}
        transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }}
      />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_15%,rgba(91,156,255,0.22),transparent_45%)]" />
      ...
      >
        ...
        <p className="mt-3 text-sm text-white/70">
          ...
          no automated sends, no autodialers, just clean visibility into manual outbound work.
        </p>
      </motion.div>
      ...
      <div className="relative z-10 mt-10 flex w-full flex-1 items-center justify-center">
        <div className="relative w-full max-w-sm">
          {/* Shadow ellipse beneath the card, breathing opposite to the float */}
          <motion.div
          ...
          >
            ...
            <motion.div
              className="absolute -inset-1 rounded-2xl bg-gradient-to-r from-blue-400/50 via-violet-400/50 to-cyan-400/50 blur-xl"
              animate={{ opacity: [0.6, 1, 0.6] }}
              transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
            />

            <div className="relative rounded-2xl border border-white/60 bg-white/95 p-8 text-[#0F1419] shadow-2xl shadow-blue-900/30 backdrop-blur-md">
