      ...
      <div className="flex flex-wrap items-center gap-2">
        {orgs.length > 1 && (
          <select
            ...
            className="h-9 rounded-lg border border-indigo-200 bg-white px-2 text-xs"
          >
      ...
      {results !== null && (
        <div className="mt-4 space-y-2">
          {results.length === 0 ? (
            ...
            results.map((obj) => (
              <div key={obj.objectName} className="rounded-lg border border-indigo-200 bg-white/60">
                <button
                  type="button"
                  onClick={() => toggle(obj.objectName)}
                  className="flex w-full items-center justify-between px-3 py-2 text-left text-xs font-medium text-indigo-900"
                >
                  <span>
                    {obj.label} <span className="text-indigo-600">({obj.objectName})</span>
                    {obj.custom && <span className="ml-2 rounded-full bg-indigo-100 px-1.5 py-0.5 text-[10px]">custom</span>}
                  </span>
                    ...
                    <ul className="space-y-1 text-[11px] text-indigo-800">
                      {obj.fields.map((f) => (
                        <li key={f.name} className="flex justify-between gap-3">
                          ...
                          <span className="text-indigo-600">{f.label} · {f.type}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
